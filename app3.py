...
try:
    ...
    if not DEPLOYMENT:
        st.error("❌ Missing deployment name in .env file")
        st.stop()
...
Return ONLY the JSON array, nothing else."""
    try:
        response = client.chat.completions.create(
            model=DEPLOYMENT,
            messages=[
                {"role": "system", "content": "You are a precise question extraction assistant. Return only valid JSON arrays."},
        ...
        return questions if isinstance(questions, list) else [query]
    except Exception as e:
...
"""
    ...
    # Add the "Information Sought" section
    response += "**Information Sought:**\n\n"
    for idx, (question, _) in enumerate(questions_and_matches, 1):
        response += f"{idx}. {question}\n\n"
    response += "\n"

    # Start the table (markdown)
    response += "S.No. | Reply\n"
    response += "---|---\n"
    ...
    for idx, (question, match) in enumerate(questions_and_matches, 1):
        if match is not None:
            ...
            # ---------- FIX: use regex-based extraction ----------
            ...
            # Ensure clickable link in Streamlit markdown (plain URL usually works; keep as-is)
            response_text_final = natural_response
        else:
            response_text_final = (
...
def main():
    ...
    st.title("TRAI RTI Response Generator - Knowledge Base Only")
    st.markdown("Generate official RTI responses using **ONLY** answers from TRAI's knowledge base (no static responses)")

    # Load knowledge base
    knowledge_base = load_knowledge_base()

    # Display knowledge base status
    st.sidebar.success(f"✅ Loaded {len(knowledge_base)} RTI themes from knowledge base")
    st.sidebar.info("💡 **Multi-Question Support:** Ask multiple questions in a single submission. Each answer comes directly from the knowledge base.")
    st.sidebar.warning("⚠️ **No Static Responses:** All answers are matched from your Excel knowledge base only.")
...
What are the QoS parameters? How do I file a complaint? Where can I find tariff information?"""
    ...
    if st.button("Generate Response", type="primary", disabled=not user_query):
        with st.spinner("Processing your query with Azure OpenAI LLM..."):
            ...
            for idx, question in enumerate(questions, 1):
                with st.spinner(f"Processing question {idx}/{len(questions)}..."):
