...
def get_aoai_client():
    """
    Cache the Azure OpenAI client for Streamlit to stabilize sessions.
    Adds explicit timeouts and limits implicit retries.
    """
    return AzureOpenAI(
        azure_endpoint=get_secret("AZURE_OPENAI_ENDPOINT"),
        api_key=get_secret("AZURE_OPENAI_API_KEY"),
        api_version="2024-02-01",  # keep your original API version
        timeout=45.0,              # explicit client timeout
        max_retries=1              # avoid long implicit retries
    )
...
def extract_first_url(text: str) -> str:
    ...
    if not text or str(text).strip() in {"—", "nan", "", "1", "None"}:
        return ""
    s = str(text).strip()
    # Discard blob: type or non-http(s)
...
# -------------------------------
# Knowledge base loader (unchanged)
# -------------------------------
...
# -------------------------------
# Question splitter (timeout added)
# -------------------------------
def split_into_questions(query):
    """Use LLM to intelligently split user query into individual questions (prompt unchanged)"""
...
Return ONLY the JSON array, nothing else. Do not include any explanatory text."""
    try:
        response = client.chat.completions.create(
            ...
            timeout=45.0       # NEW: explicit per-request timeout
        )
        result = response.choices[0].message.content.strip()

        # Clean up if code blocks present
        if "```json" in result:
        ...
        st.warning("⚠️ Could not parse LLM response as JSON. Processing as single query.")
        return [query]
    except Exception as e:
...
# -------------------------------
# LLM-based matcher (timeout added)
# -------------------------------
def find_best_match_with_llm(query, knowledge_base):
    """Use LLM to find the best matching RTI theme for the user's query (prompt unchanged)"""
...
4. Do not explain your reasoning or provide any other text"""
    try:
        response = client.chat.completions.create(
            ...
            temperature=0.1,
            max_tokens=10,     # unchanged
            timeout=45.0       # NEW
        )
        match_id_text = response.choices[0].message.content.strip()
        ...
        try:
            ...
            # Fallback: check if a theme string came back
...
# -------------------------------
# Response summarizer (timeout added)
# -------------------------------
def summarize_response_with_llm(question, response_text, url):
    """Use LLM to convert knowledge base response into natural, professional language (prompt unchanged)"""
    url_instruction = ""
    if url:
        url_instruction = (
...
# -------------------------------
# Multi-response composer (unchanged)
# -------------------------------
...
def create_response_excel(response_text, questions_and_matches):
    ...
    for idx, (question, match) in enumerate(questions_and_matches, 1):
        if match is not None:
            ...
            url = extract_first_url(raw_urls)
            natural_response = summarize_response_with_llm(question, response_text, url)
            # Make URL explicit at the end for visibility/clickability
